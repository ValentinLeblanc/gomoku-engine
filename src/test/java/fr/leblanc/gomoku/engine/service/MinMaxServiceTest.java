...
class MinMaxServiceTest {
	...
	private void testNextMoves(GameDto gameDto, int depth, int playingColor) throws InterruptedException {
		...
		MinMaxResult minMaxResult = minMaxService.computeMinMax(DataWrapper.of(gameDto), playingColor, null, depth);
		...
		for (int index = 0; index < depth; index++) {
			...
			MoveDto newMove = new MoveDto(move, playingColor);
		...
		assertEquals(evaluation, evaluationService.computeEvaluation(DataWrapper.of(gameDto), playingColor), 0.0001);
	}
