...
@SpringBootTest
class ThreatContextTest extends AbstractGomokuTest {
	...
	void testThreat5AndDoubleThreat4() throws JsonProcessingException {
		...
		ThreatContext playingThreatContext = GameData.of(gameDto).getThreatContext(GomokuColor.BLACK_COLOR);
		...
		ThreatContext opponentThreatContext = GameData.of(gameDto).getThreatContext(GomokuColor.WHITE_COLOR);
	...
	@Test
	void testUpdateThreatContext() throws JsonProcessingException {
		GameDTO gameDto = GomokuTestsHelper.readGameDto("testAddMoveUpdate.json");
		
		GameData gameData = GameData.of(gameDto);
		
		ThreatContext playingThreatContext = gameData.getThreatContext(GomokuColor.WHITE_COLOR);
		List<Threat> dThreat3List = playingThreatContext.getThreatsOfType(ThreatType.DOUBLE_THREAT_3);
		ThreatContext opponentThreatContext = gameData.getThreatContext(GomokuColor.BLACK_COLOR);
		List<Threat> threat5List = opponentThreatContext.getThreatsOfType(ThreatType.THREAT_5);
		
		assertEquals(1, threat5List.size());
		assertEquals(new Cell(10, 5), threat5List.get(0).getTargetCell());
		assertEquals(8, dThreat3List.size());
		
		gameData.addMove(new Cell(10, 5), GomokuColor.WHITE_COLOR);
		
		assertEquals(0, threat5List.size());
		assertEquals(8, dThreat3List.size());
